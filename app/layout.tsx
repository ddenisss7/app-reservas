...
function UserMenu({ email }: { email: string }) {
  ...
  const handleLogout = async () => {
    ...
    // No hace falta reload si onAuthStateChange está activo, 
    // pero lo dejamos por seguridad para limpiar caché.
        ...
        <>
          ...
          <div className="absolute right-0 z-20 mt-2 w-48 rounded-xl shadow-2xl overflow-hidden"
            ...
            <div className="px-4 py-3 border-b border-white/5">
              <p className="text-[10px] uppercase tracking-widest text-white/40 font-bold">Sesión iniciada</p>
...
export default function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  useEffect(() => {
    ...
    // Esto detecta cuando el usuario inicia sesión y actualiza el estado 'user' al instante
      ...
      >
        ...
        >
          <div style={{ maxWidth: "900px", margin: "0 auto", textAlign: "center" }}>
            <p style={{ fontFamily: "var(--font-display)", fontSize: "1.4rem", color: "var(--white)", marginBottom: "1rem" }}>
              Gourmet<span style={{ color: "var(--accent-lit)" }}>Reserve</span>
            </p>
            <p style={{ color: "var(--muted)", fontSize: "0.9rem", marginBottom: "1.5rem" }}>
              © 2026 GourmetReserve — Todos los derechos reservados.
            </p>
            <div style={{ display: "flex", justifyContent: "center", gap: "2rem", fontSize: "0.85rem", color: "var(--muted)" }}>
